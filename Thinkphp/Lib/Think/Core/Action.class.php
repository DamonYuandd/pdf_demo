    ...
    private function _dispatch_jump($message,$status=1,$ajax=false) {
        ...
        if($status) { //发送成功信息
            ...
            if(!$this->view->get('waitSecond')) {
            	...
            	$this->assign('autoURL','<script>setInterval(function(){location.href="'.$_SERVER["HTTP_REFERER"].'";},1000);</script>');
            }
            // 默认操作成功自动返回操作前页面
            if(!$this->view->get('jumpUrl')) {
            	$this->assign("jumpUrl",$_SERVER["HTTP_REFERER"]);
            	$this->assign('autoURL','<script>setInterval(function(){location.href="'.$_SERVER["HTTP_REFERER"].'";},3000);</script>');
            }
            ...
            if(!$this->view->get('jumpUrl')) {
            	...
            	$this->assign('autoURL','<script>setInterval("history.go(-1)",3000)</script>');
            }
